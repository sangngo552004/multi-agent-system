...
export function HrShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="hr-shell min-h-screen bg-canvas">
      <div className="fixed inset-y-0 left-0 z-40 hidden w-[252px] lg:block"><HrSidebar /></div>
      <Dialog.Root open={menuOpen} onOpenChange={setMenuOpen}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 z-50 bg-brand/20 backdrop-blur-[2px] lg:hidden" />
          <Dialog.Content className="fixed inset-y-0 left-0 z-50 w-[286px] max-w-[86vw] shadow-float outline-none lg:hidden">
            ...
            <Dialog.Close className="absolute right-3 top-3 grid size-9 place-items-center rounded-[8px] text-muted transition-colors hover:bg-brand/[0.06] hover:text-brand" aria-label="Đóng menu"><X className="size-4" /></Dialog.Close>
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
      <div className="lg:pl-[252px]">
        <HrTopbar onOpenMenu={() => setMenuOpen(true)} />
        <main className="ptit-hr-content mx-auto w-full max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">{children}</main>
      </div>
    </div>
  );
}
