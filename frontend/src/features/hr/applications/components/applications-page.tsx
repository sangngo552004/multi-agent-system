...
export function HrApplicationsPage() {
  ...
  const filters = useMemo<HrApplicationFilters>(() => ({ search: debouncedSearch, jobId, recruitmentStatus, dateRange, sort }), [dateRange, debouncedSearch, jobId, recruitmentStatus, sort]);
  const applications = useHrApplications(filters);
  const jobs = useHrJobs({});
  const filterCount = Number(Boolean(debouncedSearch)) + Number(jobId !== "ALL") + Number(recruitmentStatus !== "ALL") + Number(dateRange !== "ALL");
  ...
  }, [dateRange, debouncedSearch, jobId, recruitmentStatus, router, sort]);

  const clear = () => { setSearch(""); setJobId("ALL"); setRecruitmentStatus("ALL"); setDateRange("ALL"); setSort("SUBMITTED_DESC"); };
  const result = applications.data ?? [];
  const newCount = result.filter((item) => item.recruitmentStatus === "PENDING").length;

  return <div className="space-y-7">
    <PageHeader eyebrow="Hàng đợi tuyển dụng" title="Hồ sơ ứng viên" description="Theo dõi hồ sơ ứng tuyển và đưa ra quyết định cho các vị trí bạn phụ trách." />
    ...
    <section className="overflow-hidden rounded-[12px] border border-border bg-surface">
      ...
      {applications.data ? <DataTable columns={hrApplicationTableColumns} data={applications.data} getRowId={(item) => item.id} onRowClick={(item) => router.push(`/hr/applications/${item.id}`)} emptyTitle="Không có hồ sơ phù hợp" emptyDescription="Thử thay đổi vị trí, trạng thái hoặc khoảng ngày đang lọc." /> : null}
    </section>
    <p className="flex items-start gap-2 text-xs leading-5 text-muted"><CircleAlert className="mt-0.5 size-4 shrink-0 text-warning" /> Điểm đối sánh là dữ liệu hỗ trợ. HR cần đọc CV trước khi đưa ra quyết định.</p>
  </div>;
}
