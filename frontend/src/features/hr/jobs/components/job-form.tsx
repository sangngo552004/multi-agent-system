          ...
          <FormSection number="03" title="Cấu hình đối sánh" description="Chọn khung nghề nghiệp và các năng lực dùng trong đánh giá.">
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Nhóm nghề">
                <Controller control={form.control} name="jobFamilyId" render={({ field }) => <Select label="Nhóm nghề" value={field.value || "__none"} onValueChange={(value) => field.onChange(value === "__none" ? "" : value)} disabled={lockedConfiguration} options={[{ value: "__none", label: "Chọn nhóm nghề" }, ...catalog.jobFamilies.map((item) => ({ value: item.id, label: item.name }))]} />} />
              </Field>
              <Field label="Cấp bậc">
                <Controller control={form.control} name="careerLevelId" render={({ field }) => <Select label="Cấp bậc" value={field.value || "__none"} onValueChange={(value) => field.onChange(value === "__none" ? "" : value)} disabled={lockedConfiguration} options={[{ value: "__none", label: "Chọn cấp bậc" }, ...catalog.careerLevels.map((item) => ({ value: item.id, label: item.name }))]} />} />
              </Field>
            </div>
            <div className="mt-6 border-t border-border pt-5">
              <CompetencyConfiguration control={form.control} register={form.register} catalog={catalog} disabled={lockedConfiguration} />
