...
import { ArrowRight, FileUser, Loader2 } from "lucide-react";
...
export function RecentApplications({ job }: { job: HrJobDetail }) {
  const applications = useHrApplications({ jobId: job.id, sort: "SUBMITTED_DESC" });
  ...
  return (
    <section className="rounded-[12px] border border-border bg-surface" aria-labelledby="recent-applications-title">
      ...
      {applications.isPending ? <div className="flex min-h-44 items-center justify-center"><Loader2 className="size-5 animate-spin text-brand" /></div> : items.length ? <div className="divide-y divide-border">{items.map((application) => { const recruitment = recruitmentStatusMap[application.recruitmentStatus]; return <Link href={`/hr/applications/${application.id}`} key={application.id} className="group grid gap-3 px-5 py-4 transition-colors hover:bg-[#fafbf8] sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center"><div className="flex min-w-0 items-center gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-[9px] bg-surface-soft text-[10px] font-semibold text-brand">{getInitials(application.candidateName)}</span><span className="min-w-0"><span className="block truncate text-sm font-semibold text-ink">{application.candidateName}</span><span className="mt-0.5 block truncate text-[11px] text-muted">{application.candidateEmail}</span></span></div><StatusDot label={recruitment.label} tone={recruitment.tone} className="text-xs" /><div className="flex items-center justify-between gap-3 text-[11px] text-muted sm:justify-end">{application.matchScore !== undefined ? <strong className="tabular-nums text-ink">{application.matchScore}/100</strong> : null}<span>{formatRelativeTime(application.submittedAt)}</span><ArrowRight className="size-4 text-faint transition-transform group-hover:translate-x-1 group-hover:text-brand" /></div></Link>; })}</div> : <div className="flex min-h-44 flex-col items-center justify-center px-6 text-center"><FileUser className="size-6 text-faint" /><p className="mt-3 text-sm font-semibold text-ink">Chưa có hồ sơ</p><p className="mt-1 text-xs text-muted">Ứng viên mới sẽ xuất hiện tại đây.</p></div>}
    </section>
  );
}
