...
export function HrJobLifecycleActions({ job }: { job: HrJobDetail }) {
  ...
  const runStatusChange = async () => {
    ...
    try {
      ...
      toast.success(pendingAction.status === "PUBLISHED" ? "Tin đã được mở tuyển" : pendingAction.status === "PAUSED" ? "Tin đã được tạm dừng" : "Tin đã được đóng");
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-end gap-2">
        ...
        {job.status === "DRAFT" ? <Button size="sm" onClick={() => setPendingAction({ status: "PUBLISHED", title: "Mở tin tuyển dụng?", description: "Tin sẽ hiển thị cho ứng viên và bắt đầu nhận hồ sơ. Cấu hình đối sánh sẽ được khóa để giữ kết quả nhất quán.", confirm: "Mở tuyển" })}><Play className="size-4" /> Mở tuyển</Button> : null}
        {job.status === "PUBLISHED" ? <Button variant="secondary" size="sm" onClick={() => setPendingAction({ status: "PAUSED", title: "Tạm dừng nhận hồ sơ?", description: "Tin tạm thời không nhận hồ sơ mới. Các hồ sơ đã nhận và kết quả đánh giá vẫn được giữ nguyên.", confirm: "Tạm dừng" })}><Pause className="size-4" /> Tạm dừng</Button> : null}
        {job.status === "PAUSED" ? <Button size="sm" onClick={() => setPendingAction({ status: "PUBLISHED", title: "Mở lại tin tuyển dụng?", description: "Tin sẽ tiếp tục hiển thị và nhận hồ sơ mới cho đến hạn tuyển.", confirm: "Mở lại" })}><Play className="size-4" /> Mở lại</Button> : null}
        {job.status === "PUBLISHED" || job.status === "PAUSED" ? <Button variant="ghost" size="sm" className="text-danger hover:text-danger" onClick={() => setPendingAction({ status: "CLOSED", title: "Đóng tin tuyển dụng?", description: "Tin sẽ ngừng nhận hồ sơ và không thể mở lại. Bạn vẫn có thể xử lý các hồ sơ hiện có hoặc nhân bản tin để tuyển đợt mới.", confirm: "Đóng tin" })}><Square className="size-4" /> Đóng tin</Button> : null}
