...
import { JobApplicationsPanel } from "@/features/hr/jobs/components/job-applications-panel";
import { JobEmailHistoryPanel } from "@/features/hr/jobs/components/job-email-history-panel";
...
export function HrJobDetailPage({ jobId }: { jobId: string }) {
  ...
  return (
    <div className="space-y-7">
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(280px,0.72fr)]"><div className="min-w-0 space-y-6"><HrJobContentPreview job={job} /><HrJobMatchingPanel job={job} /><JobApplicationsPanel job={job} /><JobEmailHistoryPanel jobId={job.id} /></div><div className="space-y-4"><HrJobSummaryRail job={job} /></div></div>
    </div>
  );
}
