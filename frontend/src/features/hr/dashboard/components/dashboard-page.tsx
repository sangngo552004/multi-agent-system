...
export function HrDashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {dashboard.data && !dashboard.data.hasData ? <div className="rounded-[12px] border border-border bg-surface"><EmptyState title="Chưa có dữ liệu tuyển dụng" description="Bạn chưa phụ trách vị trí hoặc hồ sơ nào." /></div> : null}
      {dashboard.data?.hasData ? <div className="space-y-6"><RecruitmentPulse metrics={dashboard.data.metrics} /><HrAttentionQueue items={dashboard.data.attention} /></div> : null}
    </div>
  );
}
