...
export function AiDiagnosticsPanel({ application }: { application: ApplicationDetail }) {
  const failure = application.errorCode
    ? getAiFailurePresentation(application.errorCode)
    : null;
  ...
  return (
    <section className="overflow-hidden rounded-[12px] border border-border bg-surface">
      <header className="flex flex-col gap-3 border-b border-border px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {failure ? <Badge tone="danger">{failure.label}</Badge> : <Badge tone="success">Không có lỗi</Badge>}
      </header>
      ...
      {failure ? <div className="border-t border-border bg-danger/[0.035] px-5 py-4"><p className="text-xs font-semibold text-danger">Mô tả lỗi</p><p className="mt-1.5 text-sm leading-6 text-muted">{failure.description}</p><p className="mt-2 text-[10px] font-medium text-faint">Mã tham chiếu: {failure.reference}</p></div> : null}
