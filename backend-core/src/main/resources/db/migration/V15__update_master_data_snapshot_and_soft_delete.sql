...
ALTER TABLE jobs ADD COLUMN IF NOT EXISTS job_status VARCHAR(50) NOT NULL DEFAULT 'DRAFT';

-- 2. Thêm cột snapshot_data lưu trữ JSON snapshot
ALTER TABLE jobs ADD COLUMN IF NOT EXISTS snapshot_data TEXT;

-- 3. Thêm is_active cho các bảng Master Data (ngoại trừ institutional_rules đã có sẵn từ V6)
ALTER TABLE competencies ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE career_levels ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE job_families ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT TRUE;
